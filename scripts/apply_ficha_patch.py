...
if 'await salvarFichaAnamnese({' not in text:
    text = text.replace('    const observacaoPagamento =\n', save_block + '    const observacaoPagamento =\n')
...
if old in f:
    f = f.replace(old, new)
ficha.write_text(f, encoding='utf-8')
...
if 'const pagamentoCriado' not in p and needle in p:
    p = p.replace(needle, replacement)
payment.write_text(p, encoding='utf-8')

subprocess.run(['git','config','user.name','github-actions[bot]'], check=True)
subprocess.run(['git','config','user.email','41898282+github-actions[bot]@users.noreply.github.com'], check=True)
subprocess.run(['git','rm','scripts/apply_ficha_patch.py'], check=False)
subprocess.run(['git','add','src/routes/index.tsx','src/lib/fichas.functions.ts','src/lib/pagamento.functions.ts'], check=True)
subprocess.run(['git','commit','-m','automation: ficha persistence and payment linkage'], check=False)
