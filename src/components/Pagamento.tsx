...
export default function Pagamento({ nome, whatsapp, plano, pagamento, orderNsu, onPaid }: Props) {
  ...
  const verificar = async (nsu: string) => {
    ...
    if (result.ok) {
      ...
      if (result.status === "pago") onPaid?.(result.id);
      ...
      <div className="mt-6 rounded-2xl border border-primary/40 bg-card p-5 text-center">
        {status === "pago" ? (
          <>
            ...
            <p className="text-sm text-muted-foreground">Liberando sua agenda...</p>
          </>
        ) : (
          <>
            <Loader2 className="mx-auto h-7 w-7 animate-spin text-primary" />
            <p className="mt-2 font-semibold text-primary">Confirmando seu pagamento...</p>
            <p className="mt-1 text-sm text-muted-foreground">A agenda só será liberada quando a InfinitePay confirmar o pagamento.</p>
            <button type="button" onClick={() => void verificar(orderNsu)} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-primary underline">
  ...
  return (
    <div className="mt-6 rounded-2xl border border-primary/40 bg-accent/40 p-5">
      <div className="flex items-center gap-2 text-primary"><LockKeyhole className="h-5 w-5" /><p className="font-semibold">Pagamento seguro</p></div>
      <p className="mt-2 text-sm text-muted-foreground">O pagamento será feito diretamente no checkout da InfinitePay. Pix e cartão ficam disponíveis no próprio checkout.</p>
      {plano && <p className="mt-3 text-sm font-semibold">Plano: {plano}</p>}
      <button type="button" disabled={carregando} onClick={() => void iniciarPagamento()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-4 text-sm font-bold uppercase tracking-wide text-primary-foreground disabled:opacity-60">
