...
async function chamarAdminEdge(action: string, body: Record<string, unknown> = {}) {
  const request = getRequest(); const authorization = request?.headers.get("authorization");
...
export const listarFichasAdmin = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }): Promise<FichaAdmin[]> => { garantirAdmin(context); return (await chamarAdminEdge("fichas")) as FichaAdmin[]; });
export const atualizarFichaPagamento = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { fichaId: string; pagamentoId: string }) => input).handler(async ({ context, data }) => { garantirAdmin(context); await chamarAdminEdge("atualizar-ficha-pagamento", data); return { ok: true }; });
export const listarPagamentosAdmin = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => { garantirAdmin(context); return (await chamarAdminEdge("pagamentos")) ?? []; });
