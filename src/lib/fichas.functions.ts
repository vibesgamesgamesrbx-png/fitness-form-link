...
export const listarFichasAdmin = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }): Promise<FichaAdmin[]> => { garantirAdmin(context); return (await chamarAdminEdge(context.authorization as string | undefined, "fichas")) as FichaAdmin[]; });
export const atualizarFichaPagamento = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { fichaId: string; pagamentoId: string }) => input).handler(async ({ context, data }) => { garantirAdmin(context); await chamarAdminEdge(context.authorization as string | undefined, "atualizar-ficha-pagamento", data); return { ok: true }; });
export const listarPagamentosAdmin = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => { garantirAdmin(context); return (await chamarAdminEdge(context.authorization as string | undefined, "pagamentos")) ?? []; });
